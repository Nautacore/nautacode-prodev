  ...
  return (
    ...
    <main className="min-h-screen bg-[#0a0a0a] pt-24 pb-12 px-4 flex flex-col items-center relative overflow-hidden">
      ...
      <section className="relative z-10 bg-slate-900/80 backdrop-blur-md p-8 rounded-2xl border border-slate-700 shadow-2xl max-w-lg w-full">
        ...
        <form action="https://formspree.io/f/xaqwypry" method="POST" className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-blue-300 mb-1">Nombre de Explorador</label>
            <input name="nombre" type="text" placeholder="Ej: Neil Armstrong" className="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-blue-300 mb-1">Correo Electrónico</label>
            <input name="email" type="email" placeholder="nombre@galaxia.com" className="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-blue-300 mb-1">Mensaje a la Base</label>
            <textarea name="mensaje"rows={4} placeholder="Escribe tu reporte aquí..." className="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" required></textarea>
          </div>
